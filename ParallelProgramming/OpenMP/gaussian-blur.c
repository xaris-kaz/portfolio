...
double begin_serial=0, finish_serial=0,begin_parallel=0, finish_parallel=0, begin_tasks=0, finish_tasks=0;//serial,tasks,parrallel initialiazation
...
{		/* TODO: Implement parallel Gaussian Blur using OpenMP loop parallelization */	
	...
	begin_parallel=omp_get_wtime();//xroometrisi

	#pragma omp parallel for shared(i) private(j,row,col,redSum,greenSum, blueSum, weightSum) num_threads(4) //loop1
	for (i = 0; i < height; i++)
	{
		//#pragma omp parallel for shared(i,j) private(row,col,redSum,greenSum, blueSum, weightSum) num_threads(4) //loop2
		for (j = 0; j < width ; j++) 
		{
			//#pragma omp parallel for shared(i,j,row) private(col,redSum,greenSum, blueSum, weightSum) num_threads(4)	//loop3		
			for (row = i-radius; row <= i + radius; row++)
			{
				//#pragma omp parallel for num_threads(4) reduction(+:redSum,greenSum, blueSum, weightSum)	//loop4		
				for (col = j-radius; col <= j + radius; col++) 
				{
					int x = clamp(col, 0, width-1);
					int y = clamp(row, 0, height-1);
...
{
	int i, j;/* TODO: Implement parallel Gaussian Blur using OpenMP tasks */
	int width = imgin->header.width, height = imgin->header.height;
	double row, col;
	double weightSum = 0.0, redSum = 0.0, greenSum = 0.0, blueSum = 0.0;

	begin_tasks=omp_get_wtime();
