...
/void gaussian_blur_omp_gpu(int radius, img_t *imgin, img_t *imgout)
{
	int i, j;
	int width = imgin->header.width, height = imgin->header.height;
	double row, col;
	...
	#pragma omp target teams distribute parallel for num_threads(256) num_teams(15) private(i,j) collapse(2) map(to: imgin->red[:2250000], imgin->green[:2250000], imgin->blue[:2250000]) map(tofrom: imgout->red[:2250000], imgout->green[:2250000], imgout->blue[:2250000])
	for (i = 0; i < height; i++)
	{
		for (j = 0; j < width ; j++) 
		{
...
/*void gaussian_blur_omp_gpu(int radius, img_t *imgin, img_t *imgout)
{
	...
	#pragma omp target  map(to: imgin->red[:2250000], imgin->green[:2250000], imgin->blue[:2250000]) map(tofrom: imgout->red[:2250000], imgout->green[:2250000], imgout->blue[:2250000])
	{
		#pragma omp teams num_teams(120)
		{
			#pragma omp distribute parallel for num_threads(32) private(i,j) collapse(2)
...
int main(int argc, char *argv[]) 
{
	...
	sprintf(paroutfile_gpu, "%s-r%d-omp-loops.bmp", noextfname, radius);
	...
	exectime_omp_gpu = timeit(gaussian_blur_omp_gpu, radius, &imgin, &pimgout_gpu);
